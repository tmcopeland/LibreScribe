...
DeviceInfo::DeviceInfo(wxWindow* parent, uint16_t productID,
                       Smartpen* smartpen, wxWindowID id,const wxPoint& pos,
                       const wxSize& size)
{
    ...
    Create(parent, wxID_ANY, _("Smartpen Device Information"),
    ...
    infoIcon = new wxStaticBitmap(
            this,
            idInformationIcon,
            wxBitmap(wxImage(_T("res/dialog-information-128.png")).Rescale(wxSize(128,128).GetWidth(),
                     wxSize(128,128).GetHeight())),
                     wxDefaultPosition,
                     wxSize(128,128),
                     0,
                     _T("idInformationIcon"));
    
    mainSizer->Add(infoIcon,
                   1,
                   wxALL|
    ...
    deviceType = new wxStaticText(this, idDeviceTypeText,
                                  _("LightScribe Pulse(TM) Smartpen"),
                                  wxDefaultPosition,
                                  wxDefaultSize,
                                  wxALIGN_LEFT,
                                  _T("idDeviceTypeText"));
    
    wxFont deviceTypeFont(13, wxSWISS, wxFONTSTYLE_NORMAL, wxBOLD, false,
    ...
    nameLabel = new wxStaticText(this, idDeviceNameLabel, _("Name:"),
                                 wxDefaultPosition, wxDefaultSize, 0,
                                 _T("idDeviceNameLabel"));
...
stripNonNumericChars(const char* s)
{
...
float
DeviceInfo::convertBytesToMiB(unsigned long long int bytes)
{
...
float
DeviceInfo::getBatteryVoltage(xmlNode *root)
{
...
int
DeviceInfo::getBatteryRemaining(xmlNode *root)
{
    //first we need to locate the "peninfo" node which contains the "battery" node
    xmlNode *cur_node = getSubNode(root, (const xmlChar *)"peninfo");
    //once we find our "peninfo" node, search through the nodes, looking for the "battery" node
...
unsigned long long int
DeviceInfo::getFreeBytes(xmlNode *root)
{
...
unsigned long long int
DeviceInfo::getTotalBytes(xmlNode *root)
{
    //first we need to locate the "peninfo" node which contains the "memory" node
    xmlNode *cur_node = getSubNode(root, (const xmlChar *)"peninfo");
    //once we find our "peninfo" node, search through the nodes, looking for the "memory" node
...
void
DeviceInfo::OnKeyDownEvent(wxKeyEvent& event)
{ //FIXME does not work. needs to be fixed.
